...
*/
    ...
    public ArrayList<Artist> getAllArtist() {
        ...
        String query = "SELECT * FROM " + ARTIST_TABLE + " ORDER BY " + SortMediaStore.ArtistSortOrder.ARTIST_NAME + Constants.ASCENDING;
              //  PreferencesHelper.getInstance().getString(PreferencesHelper.Key.ARTIST_SORT_ORDER, SortMediaStore.ArtistSortOrder.ARTIST_NAME)
              //  + PreferencesHelper.getInstance().getString(PreferencesHelper.Key.ARTIST_SORT_TYPE, Constants.ASCENDING);
        Cursor cursor = getDatabase().rawQuery(query, null);
        if (cursor != null && cursor.moveToFirst()) {
            do {
                @SuppressLint("Range") Artist artist = new Artist(
                        cursor.getLong(cursor.getColumnIndex(ARTIST_ID)),
